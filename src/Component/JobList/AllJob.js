...
function AllJob() {
  ...
  const getList = async e => {
    const querySnapshot = await getDocs(
      query(collection(db, "jobs"), orderBy("created", "desc"), limit(15))
    );
    let list = [];
  ...
  return (
    <>
                    ...
                    <Link to={`/jobdetail/${sur.id}`}>
                      <div className="flex justify-between gap-3">
                        ...
                        <div className="flex flex-col justify-center basis-3/4 overflow-hidden">
                          ...
                          <div className="font-bold text-sm lg:text-base text-left truncate">
                            {sur.info.title}
                          </div>
                          <div className="flex flex-row flex-nowrap gap-3 justify-between mt-2">
                            ...
                            <div className="text-sm lg:text-base text-left">
                              {sur.info.salaryType}
                              <span className="pl-2 font-bold text-indigo-500">
                                {sur.info.salary}
                              </span>{" "}
                              원
                            </div>
      ...
      {!full ? (
        <button
          className="ease-in-out duration-300 block mx-auto hover:cursor-pointer hover:scale-110 hover:font-bold hover:text-red-500 p-3 text-sm my-2"
          onClick={e => {
            getMoreList(counter);
          }}
        >
